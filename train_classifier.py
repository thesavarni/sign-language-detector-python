...
for language in languages:
    print(f"\nTraining models for {language.upper()}")

    # Get list of data group pickle files
    data_files = sorted(glob.glob(f'./{language}_dataset_pickle/data_group_*.pickle'))
    ...
    for data_file in data_files:
        ...
        model_dir = f'./{language}_model'
        os.makedirs(model_dir, exist_ok=True)
        model_file = os.path.join(model_dir, f'model_group_{group_number}.p')
